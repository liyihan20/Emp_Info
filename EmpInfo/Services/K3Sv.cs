...
namespace EmpInfo.Services
{
    public class K3Sv:BaseSv
    {
        ...
        public List<K3BomInfo> GetK3BomInfo(int productId)
        {
            string sql = @"select 'bom' as sour, t2.FItemID as item_id,t4.FName as item_name,t4.FModel as item_model,
                        ...
                        where t1.FUseStatus = 1072 and t1.FItemID = {0}";
            sql = sql.Replace("{db}", GetConnStr());

            return db.Database.SqlQuery<K3BomInfo>(sql,productId).ToList();
        }

        //获取事业部的采购订单信息
        public List<POInfoModel> GetK3BusPO(string poNumber)
        {
            string sql = @"select t2.fentryid as entry_id,t2.fitemid as item_id,t3.fname as item_name,
